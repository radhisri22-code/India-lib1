...
import {
  doc, getDoc, updateDoc, collection, getDocs, deleteDoc, serverTimestamp
} from 'firebase/firestore';
import {
  FiArrowLeft, FiEdit3, FiTrash2, FiYoutube, FiVideo, FiUpload,
  FiUsers, FiPlay, FiMonitor, FiPlus, FiCheck, FiX, FiEye,
  FiEyeOff, FiBook, FiClock, FiDollarSign, FiGlobe,
  FiSave, FiExternalLink
} from 'react-icons/fi';
...
const TeacherCourseManage = () => {
  ...
  // Add-content panel
  const [addTab, setAddTab] = useState('youtube'); // 'youtube' | 'drive'
  ...
  const fetchAll = async () => {
    try {
      ...
      // Load live sessions (sub-collection) — sort client-side to avoid index requirement
      const sesSnap = await getDocs(collection(db, `courses/${courseId}/liveSessions`));
      const sesList = sesSnap.docs
  ...
  const togglePublish = async () => {
    ...
    try {
      ...
      toast(next ? 'Course published! Students can now enroll.' : 'Course set to draft.', 'success');
    } catch (err) {
  ...
  // ─── Save basic info edit ─────────────────────────────────────────────────
  const saveEdit = async () => {
    if (!editData.title || !editData.description || !editData.category) {
      toast('Title, description and category are required', 'error');
      return;
    }
    setSaving(true);
    try {
      ...
      setEditMode(false);
      toast('Course info updated!', 'success');
    } catch (err) {
  ...
  const addYouTube = () => {
    ...
    const updated = [...content, { type: 'youtube', title: newTitle.trim(), url: embedUrl, videoId: match[1], order: content.length }];
    saveContent(updated);
    setNewTitle('');
    setNewUrl('');
  };
  ...
  const uploadDrive = async () => {
    if (!newTitle.trim() || !videoFile) { toast('Title and video file required', 'error'); return; }
    setUploading(true);
    setUploadProgress(0);
    try {
  ...
  // ─── Persist content array to Firestore ──────────────────────────────────
  const saveContent = async (updated) => {
    setContent(updated);
    try {
      await updateDoc(doc(db, 'courses', courseId), {
        content: updated,
        contentCount: updated.length,
    ...
    <div className="tcm-page">
      ...
      <div className="tcm-topbar">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            className={`btn btn-sm ${course.isPublished ? 'btn-secondary' : 'btn-success'}`}
            onClick={togglePublish} disabled={saving}
          >
            {course.isPublished ? <><FiEyeOff size={14} /> Unpublish</> : <><FiEye size={14} /> Publish</>}
          </button>
          <Link to={`/live?course=${courseId}`} className="btn btn-danger btn-sm">
      ...
      <div className="tcm-hero">
        ...
        <div className="tcm-hero-info">
          ...
          <div className="tcm-stats-row">
            ...
            <span><FiMonitor size={14} /> <strong>{sessions.length}</strong> live sessions</span>
            <span><FiClock size={14} /> {course.duration || 'N/A'}</span>
            <span><FiBook size={14} /> {course.language}</span>
        ...
        <div className="tcm-edit-panel card">
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <select className="form-input" value={editData.level}
            ...
            <div className="form-group">
              ...
              <input className="form-input" value={editData.language}
                onChange={e => setEditData(p => ({ ...p, language: e.target.value }))} />
            </div>
            <div className="form-group">
              <label className="form-label">Duration</label>
              <input className="form-input" placeholder="e.g. 10 hours" value={editData.duration}
                onChange={e => setEditData(p => ({ ...p, duration: e.target.value }))} />
            </div>
            <div className="form-group">
              <label className="form-label">Price (₹)</label>
              <input className="form-input" type="number" min={0} value={editData.price}
      ...
      <div className="tcm-body">
        ...
        <div className="tcm-main">
            ...
            {showAddPanel && (
              <div className="tcm-add-panel">
                <div className="content-tabs">
                  <button className={`tab-btn ${addTab === 'youtube' ? 'active' : ''}`} onClick={() => setAddTab('youtube')}>
                    <FiYoutube size={15} /> YouTube Link
                  </button>
                  <button className={`tab-btn ${addTab === 'drive' ? 'active' : ''}`} onClick={() => setAddTab('drive')}>
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" placeholder="e.g. Introduction to Variables"
                ...
                {addTab === 'youtube' ? (
                  <div className="flex gap-2">
                    <input className="form-input" placeholder="https://youtube.com/watch?v=..."
                      value={newUrl} onChange={e => setNewUrl(e.target.value)} style={{ flex: 1 }} />
                    <button className="btn btn-primary" onClick={addYouTube}>
                      <FiPlus size={15} /> Add
                    </button>
                  </div>
                ) : (
                  <>
                    <div className="file-drop" onClick={() => fileInputRef.current?.click()}>
                      {videoFile
                        ? <><FiVideo size={22} /><span>{videoFile.name}</span><small>{(videoFile.size / 1e6).toFixed(1)} MB</small></>
                        : <><FiUpload size={22} /><span>Click to select video</span><small>MP4, WebM, MOV</small></>}
                ...
                {content.map((item, i) => (
                  <div key={i} className="tcm-content-item">
                    ...
                    <div className="tcm-lesson-actions">
                      {item.url && (
                        <a href={item.url} target="_blank" rel="noreferrer" className="btn btn-sm btn-secondary">
          ...
          <div className="tcm-section card">
            ...
            {sessions.length === 0 ? (
              <div className="tcm-empty">
                <FiMonitor size={32} />
                <p>No live sessions yet. Go live to teach your students!</p>
              </div>
            ) : (
                  ...
                  <div key={session.id} className={`tcm-session-item ${session.isLive ? 'is-live' : ''}`}>
                    <div className="tcm-session-icon">
                      {session.isLive
                        ...
                        : <FiMonitor size={18} color="#ef4444" />}
                    </div>
                    <div className="tcm-session-info">
                      <span className="tcm-session-title">
                        {session.title || `Live Session ${sessions.length - i}`}
                      </span>
                      <span className="tcm-session-date text-muted text-xs">
                        {formatDate(session.startedAt)}
                        {session.hasRecording && ' · Recording saved'}
                      </span>
                    </div>
                    ...
                    {session.hasRecording && session.recordingUrl && (
                      <a href={session.recordingUrl} target="_blank" rel="noreferrer" className="btn btn-sm btn-secondary">
                        <FiPlay size={13} /> View
                      </a>
                    )}
                  </div>
        ...
        <div className="tcm-sidebar">
          <div className="card tcm-stat-card">
            ...
            <div className="tcm-stat-item">
              <FiVideo size={16} style={{ color: '#f59e0b' }} />
              <span>{content.length} Total Lessons</span>
            </div>
            <div className="tcm-stat-item">
              <FiMonitor size={16} style={{ color: '#ef4444' }} />
          ...
          <div className="card" style={{ padding: '1.25rem' }}>
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                className={`btn w-full ${course.isPublished ? 'btn-secondary' : 'btn-success'}`}
                onClick={togglePublish} disabled={saving}
              >
